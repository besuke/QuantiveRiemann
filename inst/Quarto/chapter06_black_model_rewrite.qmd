...
この章では、Python / QuantLib notebook の Blackモデル章を、`QuantiveRiemann` と `QuantLib(SWIG for R)` を前提に R で書き直す。

扱う内容は次の通りである。

1. Black calculator  
2. Black process + analytic European pricing  
3. Monte Carlo European pricing  
...
- `utility.R`, `wrapper_lib.R`, `option.R` などの関数が使える
- QuantLib SWIG build で Black / BSM / path generator / Sobol / binomial engine が利用可能
...
まず、`BlackCalculator()` を直接使って NPV と Greeks を確認する。  
先物価格、行使価格、ボラティリティ、リスクフリーレートを明示的に与える。
...
次に、`BlackProcess()` と `AnalyticEuropeanEngine()` を使って、European option を評価する。

```{r black-process-analytic-european}
...
ここでは、`GaussianPathGenerator()` を使い、European call の terminal payoff を手作業で平均して価格を求める。

```{r monte-carlo-european}
n_step_mc <- 3L
n_path_mc <- 100000L
seed_mc <- 1L
...
パス本数を 1200 本に増やした場合の描画である。

```{r monte-carlo-paths-1200}
maturity_year_plot <- 3
n_step_path <- 12L
n_path_plot <- 1200L
seed_path <- 1L
...
最初のパスについて、乱数列から手計算で価格パスを再現する。

```{r one-path-hand-check}
n_step_small <- 3L
n_path_small <- 10L
seed_small <- 1L
...
Black-Scholes-Merton process を作り、European put と American put を比較する。
...
Low-discrepancy sequence を使って Sobol path を作る。
...
Sobol path を使い、American put の Longstaff-Schwartz 型の後ろ向き計算を手作業で行う。

```{r longstaff-schwartz-hand}
...
```bash
quarto render chapter06_black_model_rewrite.qmd
```
